## 👋 Hi, I'm David

I'm a computer science student who enjoys building things that sit at the intersection of **ML**, **backend systems**, and **real-world problem solving**.  
I like working close to fundamentals, but I also enjoy shipping complete, usable products.

---

## 🛠️ Impactful Projects

| Project | Description | Tech |
|-------|------------|------|
| **ExamIQ** | End-to-end exam paper generation platform with role-based access for admins and teachers, section-wise locking, and submission validation | ![Next.js](https://img.shields.io/badge/Next.js-black) ![MongoDB](https://img.shields.io/badge/MongoDB-green) ![TypeScript](https://img.shields.io/badge/TypeScript-blue) |
| **ProjectSphere** | Centralized repository for student projects with categorization, search, and peer reviews | ![Supabase](https://img.shields.io/badge/Supabase-green) ![AWS](https://img.shields.io/badge/AWS-orange) |
| **PlaceMate** | AI-powered placement preparation platform with personalized study plans, AI-generated notes, YouTube resources, MCQ practice, and progress tracking | ![Next.js](https://img.shields.io/badge/Next.js-black) ![FastAPI](https://img.shields.io/badge/FastAPI-009688) ![Firebase](https://img.shields.io/badge/Firebase-FFCA28) ![Gemini](https://img.shields.io/badge/Gemini-4285F4) |
| **Morning News Bot** | AI-powered daily news digest that intelligently ranks the most important stories, generates concise summaries, and automatically emails a curated newsletter every morning | ![Node.js](https://img.shields.io/badge/Node.js-green) ![Gemini](https://img.shields.io/badge/Gemini-blue) ![NewsAPI](https://img.shields.io/badge/NewsAPI-red) ![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-black) |
| **License Plate Detection** | Real-time license plate detection and text extraction from traffic videos using YOLO | ![YOLO](https://img.shields.io/badge/YOLO-red) ![OpenCV](https://img.shields.io/badge/OpenCV-green) |

---

## 📊 GitHub Stats

<p>
  <img src="https://streak-stats.demolab.com?user=davidAlmei&theme=dark&hide_border=true" />
</p>

---

## ⚙️ Tech I Work With

### 🧑‍💻 Languages
![Java](https://img.shields.io/badge/Java-red)
![Python](https://img.shields.io/badge/Python-blue)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow)
![TypeScript](https://img.shields.io/badge/TypeScript-blue)
![SQL](https://img.shields.io/badge/SQL-lightgrey)

---

### 🛠️ Frameworks & Tools
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-brightgreen)
![React](https://img.shields.io/badge/React-blue)
![MongoDB](https://img.shields.io/badge/MongoDB-green)
![OpenCV](https://img.shields.io/badge/OpenCV-green)
![YOLO](https://img.shields.io/badge/YOLO-red)
![Scikit-learn](https://img.shields.io/badge/Scikit--Learn-orange)

---

### 🧠 Concepts
![Backend](https://img.shields.io/badge/Backend%20Systems-black)
![REST](https://img.shields.io/badge/REST%20APIs-lightgrey)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-orange)
![Deep Learning](https://img.shields.io/badge/Deep%20Learning-orange)
![NLP](https://img.shields.io/badge/NLP-blue)
![Auth](https://img.shields.io/badge/Auth%20%26%20Security-red)

---

## 🚀 Current Endeavors

- Learning Spring Boot and backend architecture deeply  
- Exploring System Design, NLP, and model internals
- Making open-source contributions  

---

## 🤝 Open to

- Backend / ML heavy systems  
- Open-source collaboration  
- Research-oriented projects  
- Learning from strong engineers  

---

## 📬 Let's Connect

- 📧 Email: **05davidalmeida@gmail.com**
- 💼 LinkedIn: https://www.linkedin.com/in/davidalmeida200005/

I'm always open to discussing ideas, projects, and collaborations.

---

_Thanks for stopping by._
